...
const saveZone = () => {
    if (isEditMode.value) {
        ...
        // Lógica de Creación: Protegido contra lista vacía usando Math.max(..., 0) + 1
...
const goToClassrooms = (zone) => {
    // Usa 'admin-aulas' (definido en routes.js)
    router.push({ 
        name: 'admin-aulas', 
        params: { zoneId: zone.id }, // Parámetro de ruta
        query: { zoneName: zone.name } // Parámetro de consulta para mostrar en la URL
    });
};
</script>
