<template>
  <q-page class="q-pa-lg bg-grey-2">
    ...
    <q-breadcrumbs class="text-secondary q-mb-md">
      ...
      <q-breadcrumbs-el :label="currentZoneName" icon="map" />
    </q-breadcrumbs>
    ...
    <q-list bordered class="rounded-borders bg-white shadow-1">
      ...
      <q-item v-for="aula in filteredAulas" :key="aula.id" clickable v-ripple @click="goToItems(aula.id)"
        class="q-py-md aula-item">
        <q-item-section avatar>
          <q-icon :color="aula.item_count > 0 ? 'primary' : 'grey-5'" name="meeting_room" size="lg" />
        </q-item-section>
        ...
        <q-item-section side>
          <q-btn flat round :icon="aula.item_count > 0 ? 'arrow_forward' : 'block'"
            :color="aula.item_count > 0 ? 'primary' : 'grey-5'" size="md">
